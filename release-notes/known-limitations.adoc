...
Se você deseja gerenciar um cluster em outra instância do Astra Control Center, primeiro link:../use/unmanage.html#unmanage-a-cluster["desgerenciar o cluster"] remova-o da instância na qual ele está gerenciado antes de gerenciá-lo em outra instância. Após remover o cluster do gerenciamento, verifique se o cluster está desgerenciado executando este comando:
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["Instale a versão mais recente do Astra Trident"^].
...
Certifique-se de link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["introduza os valores corretos"] ao estabelecer uma conexão.
