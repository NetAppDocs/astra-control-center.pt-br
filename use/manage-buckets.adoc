...
NOTE: Ao adicionar um bucket, selecione o provedor de bucket correto e forneça as credenciais adequadas para esse provedor. Por exemplo, a interface do usuário aceita NetApp ONTAP S3 como tipo e aceita credenciais do StorageGRID; no entanto, isso fará com que todos os futuros backups e restaurações de aplicativos usando esse bucket falhem. Consulte o link:../release-notes/known-issues.html["Notas de versão"].
