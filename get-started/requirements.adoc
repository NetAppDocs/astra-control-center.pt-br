...
+
** O token do arquivo de configuração padrão do VMware TKG e TKGi expira dez horas após a implantação. Se você usa produtos do portfólio Tanzu, é necessário gerar um arquivo de configuração do Tanzu Kubernetes Cluster com um token que não expira para evitar problemas de conexão entre Astra Control Center e os clusters de aplicativos gerenciados. Para obter instruções, visite  https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-2.html["Documentação do produto VMware NSX-T Data Center"^].
